 ...
 */
export async function createLobby(): Promise<APIResult<CreateLobbyRequest>> {
  try {
    const response = await post("/api/lobby", {});
    const json = (await response.json()) as CreateLobbyRequest;
...
export async function getMessages(id: string): Promise<APIResult<Message[]>> {
  try {
